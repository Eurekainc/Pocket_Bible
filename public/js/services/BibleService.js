...
var BibleService = function () {
    this.initialize = function() {
		...
		window.localStorage.setItem("bible", JSON.stringify(
			[
			 {
			   "bid": 1,
			   "book_name": "Genesis",
	...
	this.chapterTable = function(book){
		...
		var row = '';
		var table = '<div class = "bible_background"> <h1 class = "bible_book_heading" align = "center">' + book.book_name +'</h1><p class ="bible_select_chapters" align = "center" >Select Chapter<p><table class = "bible_chapters"><tr>';
		var chapter = '';
		for (var i = 1; i <= chap; i++) {
			chapter = 'book' + book.bid + '--chapter' + i + '.txt';
			row = table + '<td class = "bible_chapter"><a class = "bible_link" href = "#chapter/' + chapter + '">' + i +'</a></td>';
			table = row;
			if (i%5 == 0){
				row = table + '</tr><tr>';
				table = row;
	...
	this.findAllBooks= function(){
		...
		//results = lessons.sort(SortByNumber);
		deferred.resolve(results);
		return deferred.promise();
    ...
    this.findChapterById = function(id){
		...
		if (!GetBibleChapterFromStorage('bible_' + id)) {
			...
			JSZipUtils.getBinaryContent("http://127.0.0.1:8000/bible/" + book_name, function(err, data) {
				if(err) {
					chapter = "Check your Internet State.";
					deferred.resolve(chapter);
				}
			
				JSZip.loadAsync(data).then(function (zip) {
					Object.keys(zip.files).forEach(function(file_name) {
						zip.file(file_name).async('text').then(function(file_content) {
							chapter = SetBibleChapterToStorage('bible_' + file_name, file_content);
							deferred.resolve(chapter);
						});
					});
				});
